...
bool updateclientteam(int cln, int newteam, int ftr)
{
    ...
    if( mastermode == MM_PUBLIC && cl.state.forced && ftr == FTR_PLAYERWISH &&
...
//        if(m_lms) lmscheck();
        sendextras();
        if ( scl.afk_limit && mastermode == MM_PUBLIC && next_afk_check < servmillis && gamemillis > 20 * 1000 ) check_afk();
    }
